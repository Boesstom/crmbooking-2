<script>
  ...
  function getKanbanColumns() {
    ...
    return {
      'Akan Datang': fb.filter(b => (b.status || 'Akan Datang') === 'Akan Datang'),
      'Sedang Berlangsung': fb.filter(b => (b.status || 'Akan Datang') === 'Sedang Berlangsung'),
      'Selesai': fb.filter(b => (b.status || 'Akan Datang') === 'Selesai')
    };
  }
  ...
  async function handleDrop(event, targetColumn) {
    ...
    if (currentDraggedBooking && currentDraggedBooking.id && targetColumn) {
      try {
        ...
        toastMessage = 'Gagal mengubah status booking. Coba lagi.';
        toastType = 'error';
        showToast = true;
        setTimeout(() => (showToast = false), 3000);
      }
...
<div class="min-h-[calc(100vh-100px)] mt-10 w-full max-w-none p-0">
      ...
      {#if !loading && bookings.length > 0}
        <div class="flex items-center gap-4 mt-2 text-sm text-gray-400">
          <span class="flex items-center gap-1">
            ...
            {bookings.filter(b => getStatusText(b.status) === 'Akan Datang').length} Akan Datang
          </span>
          <span class="flex items-center gap-1">
            <div class="w-2 h-2 bg-blue-500 rounded-full"></div>
            {bookings.filter(b => getStatusText(b.status) === 'Sedang Berlangsung').length} Sedang Berlangsung
          </span>
          <span class="flex items-center gap-1">
            <div class="w-2 h-2 bg-green-500 rounded-full"></div>
            {bookings.filter(b => getStatusText(b.status) === 'Selesai').length} Selesai
          </span>
        </div>
      {/if}
    </div>
  ...
  <div class="mb-8 px-5 max-w-6xl mx-auto">
    ...
    <div class="flex flex-wrap gap-5 items-end mb-5 bg-gray-900 p-5 rounded-lg border border-gray-700">
      ...
      <div class="flex flex-col gap-1 min-w-[200px] flex-1">
        ...
        <input type="text" id="search" placeholder={t('search_placeholder')} bind:value={searchTerm} class="bg-gray-950 border border-gray-700 rounded-md px-3 py-2 text-gray-200 text-sm h-[46px]" />
      </div>
      <div class="flex gap-2 items-center h-[34px]" role="group" aria-labelledby="bulk-actions-label">
        <span id="bulk-actions-label" class="sr-only">Actions</span>
  ...
  {#if showForm}
    <div class="fixed inset-0 bg-black/70 flex items-center justify-center z-[1000]" on:click={() => showForm = false} role="dialog" aria-modal="true" aria-label="Tambah Booking" tabindex="-1" on:keydown={(e) => { if (e.key === 'Escape') showForm = false; }}>
      ...
      <div class="bg-gray-900 rounded-lg p-8 w-11/12 max-w-lg border border-gray-700" on:click|stopPropagation on:keydown|stopPropagation role="document">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-xl text-gray-200">{t('add_new_booking')}</h2>
        ...
        <form class="flex flex-col gap-5" on:submit|preventDefault={addBooking}>
          <div class="flex flex-col gap-2">
            <label for="newContactId" class="text-sm font-medium text-gray-200">{t('client_name')}</label>
            <select id="newContactId" bind:value={formData.contact_id} required class="bg-gray-950 border border-gray-700 rounded-md px-3 py-3 text-gray-200 text-sm cursor-pointer focus:outline-none focus:border-green-500 transition-colors">
              <option value="">{t('choose_contact') || 'Pilih Kontak'}</option>
          ...
          <div class="flex flex-col gap-2">
            <label for="newEmployeeId" class="text-sm font-medium text-gray-200">{t('employee')} (Opsional)</label>
            <select id="newEmployeeId" bind:value={formData.employee_id} class="bg-gray-950 border border-gray-700 rounded-md px-3 py-3 text-gray-200 text-sm cursor-pointer focus:outline-none focus:border-green-500 transition-colors">
              <option value="">{t('choose_employee') || 'Pilih Pekerja'}</option>
          ...
          <div class="flex flex-col gap-2">
            <label for="newDate" class="text-sm font-medium text-gray-200">{t('date')}</label>
            <DatePicker id="newDate" bind:value={formData.date} required returnType="string" placeholder="YYYY-MM-DD" />
          </div>
          <div class="flex flex-col gap-2">
            <label for="newPackageName" class="text-sm font-medium text-gray-200">{t('package_name')}</label>
            ...
            >
              <option value="">{t('choose_package') || 'Pilih Pakej'}</option>
          ...
          <div class="flex flex-col gap-2">
            <label for="newAmount" class="text-sm font-medium text-gray-200">{t('amount')} (RM)</label>
          ...
          <div class="flex flex-col gap-2">
            <label for="newStatus" class="text-sm font-medium text-gray-200">Status</label>
          ...
          <div class="flex gap-3 justify-end mt-2.5">
            <button type="button" class="bg-none border border-gray-700 text-gray-400 px-6 py-3 rounded-md text-sm hover:border-green-500 hover:text-green-500 transition-colors" on:click={() => showForm = false}>{t('cancel')}</button>
  ...
  {#if showDeleteConfirm}
    <div class="fixed inset-0 bg-black/70 flex items-center justify-center z-[1000]" on:click={() => showDeleteConfirm = false} role="dialog" aria-modal="true" aria-label="Konfirmasi Hapus" tabindex="-1" on:keydown={(e) => { if (e.key === 'Escape') showDeleteConfirm = false; }}>
      <!-- svelte-ignore a11y_no_noninteractive_element_interactions -->
      ...
      {:else if viewMode === 'kanban'}
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mt-5 transition-opacity duration-300 {kanbanFadeTransition ? 'opacity-50' : 'opacity-100'}">
          {#each ['Akan Datang', 'Sedang Berlangsung', 'Selesai'] as column}
            <div 
              class="bg-gray-900 rounded-lg p-5 border border-gray-700 min-h-[400px] transition-all relative {dragOverColumn === column ? 'bg-gray-950 border-green-500 scale-[1.01] shadow-inner' : ''} {isKanbanRefreshing ? 'animate-kanbanPulse' : ''} {kanbanFadeTransition ? 'animate-refreshGlow' : ''}" 
            ...
            >
              <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-semibold text-gray-200 flex items-center gap-2" style="color: {getStatusColor(column)}">
                  <Calendar size={16} />
                  {column}
                ...
                <span class="text-xs px-2 py-1 rounded bg-gray-800 border border-gray-700 text-gray-300 transition-all">
                  {kanbanColumns[column].length} booking
                  {#if isKanbanRefreshing}
                    <RefreshCw size={12} class="inline ml-1 animate-spin" />
                  {/if}
                </span>
              </div>
              {#each kanbanColumns[column] as booking (booking.id)}
                <div 
                  ...
                  style="border-left: 3px solid {getStatusColor(column)}; animation-delay: {kanbanColumns[column].indexOf(booking) * 50}ms"
                  role="listitem"
                ...
                >
                  <div class="flex items-start gap-2 mb-3">
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center justify-between gap-2">
                        ...
                        <button class="bg-transparent border-none text-red-500 p-1 rounded hover:bg-red-500/10 transition-colors" on:click={() => deleteBookingHandler(booking.id)} title={t('delete')}>
                          <Trash2 size={16} />
                        </button>
                      </div>
                  ...
                  <div class="flex justify-end gap-1 pt-1">
                    <button class="bg-transparent border-none text-gray-400 p-1 rounded hover:bg-white/10 transition-colors" on:click={() => openViewModal(booking)} title={t('view')}>
                      <Eye size={16} />
                    </button>
                    <button class="bg-transparent border-none text-gray-400 p-1 rounded hover:bg-white/10 transition-colors" on:click={() => openEditModal(booking)} title={t('edit')}>
                      <Edit size={16} />
                    </button>
                  </div>
                </div>
